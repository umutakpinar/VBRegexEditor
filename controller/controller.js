...
require(['vs/editor/editor.main'], function () {
    ...
    findText.addEventListener('input', () => {
        var alertFixer = document.getElementsByClassName('context-view fixed bottom right');
        if (alertFixer[0] != null || alertFixer[0] != undefined) {
            let display = window.getComputedStyle(alertFixer[0]).display;
            if (display == "block") {
                if (alertFixer[0].innerText != "") {
                    alertMessage = alertFixer[0].innerText;
                }
                alertFixer[0].innerText = "";
                createAlertMessage("alert-danger", alertMessage);
            } else {
                showPositiveAlertBox();
            }
        } else {
            showPositiveAlertBox();
        }
...
function createAlertMessage(msgType = "alert-warning", message = "") {
    alertBox.innerHTML = "";
    alertBox.innerHTML = `
    <div class="alert ${msgType} alert-dismissible fade show p-2 ps-4" role="alert" style="position: fixed; right: 3vw; bottom: 5vw;">
        ${message}
...
function showPositiveAlertBox() {
    let matches = document.querySelector('div.matchesCount');
    if (matches != null || matches != undefined) {
        let matchCount = matches.innerText;
        if (matchCount != "No results") {
            createAlertMessage("alert-success", matchCount.split("of ")[1] + " matches found.");
        } else {
            createAlertMessage("alert-warning", "No matches found.")
        }
    }
}
